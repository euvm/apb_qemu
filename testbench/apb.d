...
class apb_fifo_seq(uint ADDRW, uint DATAW): apb_seq!(ADDRW, DATAW)
{
  ...
  tr_frame* read_data(File fifo, uint size) {
    ubyte[] trSizeBuf;
    ...
    frame = cast(tr_frame*)trSizeBuf.ptr;

    if (_swapRequired) {
      frame.length = swapEndian(frame.length);
      frame.addr = swapEndian(frame.addr);
      frame.data = swapEndian(frame.data);
    }
  ...
  bool get_req_from_fifo(File fifo) {
    ...
    ubyte[16] rawtr_header;
    tr_header* header;

	
    writeln("Waiting for data from fifo");
    fifo.rawRead(rawtr_header);

    header = cast(tr_header*) rawtr_header;

    writeln("Read data from fifo");

    version(LittleEndian) {
      if (header.endian == 0xF0) {
	_swapRequired = true;
      }
      else if (header.endian != 0x0F) {
	writefln("Expected Delimiter 0x0F or 0xF0; got 0x%x",
		 header.endian);
      }
    }
    version(BigEndian) {
      if (header.endian == 0x0F) {
	_swapRequired = true;
      }
      else if (header.endian != 0xF0) {
	writefln("Expected Delimiter 0x0F or 0xF0; got 0x%x",
		 header.endian);
      }
    }

    uint pkt_len = header.pkt_len;
    if (_swapRequired) {
    ...
    writeln("rawtr_header is: ", rawtr_header);
    writeln("header is: ", header);
    writeln("Packet length is: ", pkt_len);

    this._kind = cast(kind_e) header.op;

    if (this._kind == kind_e.WRITE)
      {
	tr_frame* frame = read_data(fifo, pkt_len);
	writeln("inside write of req_thread");
	this._addr = cast(UBit!8) frame.addr.toBitVec ;
	this._data = cast(UBit!32) frame.data.toBitVec;
	writeln("Write Data is: ", frame.data);
	writeln("Write Data is: ", frame.data.toBitVec);
	writeln(this.sprint);
      }
  ...
  override void body() {
    ...
    req = apb32_rw.type_id.create("req_"
			   ~ get_name);
...
class apb_socket_seq(uint ADDRW, uint DATAW): apb_seq!(ADDRW, DATAW)
{
  ...
  tr_frame* read_data(Socket socket, uint size) {
    ubyte[] trSizeBuf;
    ubyte[] buffer;
	...
	uvm_fatal("SOCKET", "Error receiving data from socket");
      if (ret == 0)
	uvm_fatal("SOCKET", "Got insufficient bytes for tr_header");
  ...
  bool get_req_from_socket(Socket socket) {
    ...
    while (rawtr_header.length < 16) {
      import std.string: format;

      buffer.length = 16 - rawtr_header.length;
      auto ret = socket.receive(buffer);
    ...
    if (this._kind == kind_e.WRITE)
      {
	tr_frame* frame = read_data(socket, pkt_len);
	writeln("inside write of req_thread");
	this._addr = cast(UBit!8) frame.addr.toBitVec ;
	this._data = cast(UBit!32) frame.data.toBitVec;
	writeln("Write Data is: ", frame.data);
